...
void 
fuseserver_symlink(fuse_req_t req, const char* path, fuse_ino_t parent, const char* name)
{
	...
	ret = yfs->symlink(parent_inum, name, path, inum);  // inum will return from yfs
	// deal with result, if not OK, result can be exist or enoent
	if(ret != yfs_client::OK){
		if(ret == yfs_client::EXIST){
			fuse_reply_err(req, EEXIST);
		}else{
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
	...
	// the second parameter is const char*, so here transfer string into c style char array
